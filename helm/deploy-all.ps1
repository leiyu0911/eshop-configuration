...
if ($deployInfrastructure) {
    foreach ($infra in $infras) {
        ...
        helm install --namespace eshop --values app.yaml --values inf.yaml --values $ingressValuesFile --set app.name=$appName --set inf.k8s.dns=$dns --set ingress.hosts[0]=$dns --name="$appName-$infra" $infra     
    }
}
else {
...
if ($deployCharts) {
    foreach ($chart in $charts) {
        ...
        if ($useCustomRegistry) {
            helm install --namespace eshop --set inf.registry.server=$registry --set inf.registry.login=$dockerUser --set inf.registry.pwd=$dockerPassword --set inf.registry.secretName=eshop-docker-scret --values app.yaml --values inf.yaml --values $ingressValuesFile --set app.name=$appName --set inf.k8s.dns=$dns --set ingress.hosts[0]=$dns --set image.tag=$imageTag --set image.pullPolicy=Always --name="$appName-$chart" $chart 
        }
        else {
            if ($chart -ne "eshop-common")  {       # eshop-common is ignored when no secret must be deployed
                helm install --namespace eshop --values app.yaml --values inf.yaml --values $ingressValuesFile --set app.name=$appName --set inf.k8s.dns=$dns  --set ingress.hosts[0]=$dns --set image.tag=$imageTag --set image.pullPolicy=Always --name="$appName-$chart" $chart 
            }
        }
    }
}
